...
def save(request):
    ...
    a.save()
    b.save()
    print('Your data has successfully submitted')
...
def complaintsave(request, id):
    ...
    v = Complaints(name=sg.name, program=sg.course, branch=sg.branch, contactno=sg.mobile,
                    email=sg.email, Subject=Subject, comp=comp,status=status, reqdate=reqdate)
    v.save()
